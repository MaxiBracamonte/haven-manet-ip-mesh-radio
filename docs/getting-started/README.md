...
1. **Flash** each node with firmware for its platform and radio. **Haven 1 / Pi 4 or CM4** uses a current [OpenMANET](https://openmanet.org/) image. **Haven 2 / Pi 5** requires the Pi 5-specific OpenWrt/Morse firmware from [`buildwithparallel/openwrt-morse-rpi5`](https://github.com/buildwithparallel/openwrt-morse-rpi5), or the prebuilt [v0.3.0-alpha release](https://github.com/buildwithparallel/openwrt-morse-rpi5/releases/tag/v0.3.0-alpha). Use Raspberry Pi Imager; if the card misbehaves, see the SD card tips in the setup guide.
